...
from data.load_mnist_binary import load_mnist
...
def run():
    X_train, X_test, y_train, y_test = load_mnist(minority_ratio=0.02)
    ...
    plot_confusion(
        ...
        y_pred=y_pred,
        labels=["Non-0", "0"],
        title="MNIST Binary – Class Weights",
        filename="mnist_confusion.png",
    )

    plot_roc_pr(
        y_true=y_true,
        y_score=y_score,
        title_prefix="MNIST Binary",
        filename="mnist_roc_pr.png",
    )
